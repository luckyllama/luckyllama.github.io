...
var SortingView = (function () {
    function SortingView($el, display) {
        if (typeof display === "undefined") { display = SvgDisplay; }
        this.$el = $el;
...
var D3Display = (function () {
    ...
    D3Display.prototype._setupItemLine = function () {
        ...
        this.itemLine = d3.svg.line().interpolate("cardinal").tension(.99).x(function (d, i) {
            return _this.x(i);
        }).y(function (d) {
            return _this.y(d);
        });
    ...
    D3Display.prototype._setItemData = function () {
        ...
        item.append("path").attr("d", function (d) {
            return _this.itemLine(d.positions);
        }).style("stroke", function (d) {
            return baseColor.darker(colorDelta * parseInt(d.name));
        });
    };
    ...
    D3Display.prototype._setStepAreas = function () {
        ...
        var step = steps.selectAll(".step").data(this.logs).enter().append("g").attr("class", "step").attr("data-value", function (d) {
            return d.join(", ");
        });

        step.append("rect").attr("class", "step-area").attr("x", function (d, i) {
            return _this.x(i);
        }).attr("y", -1).attr("width", this.stepWidth).attr("height", this.$el.height() + 2);

        this.svg.on("mousemove", function () {
            var x = Math.floor(_this.x.invert(d3.mouse(_this.svg.node())[0]));
            step.classed("active", function (d, i) {
                return i === x;
            });
    ...
    D3Display.prototype._setStepPoints = function () {
        ...
        var createCircles = function (el, d, i) {
            ...
            el.selectAll(".point").data(d).enter().append("circle").attr("cx", function (d) {
                return _this.x(i);
            }).attr("cy", function (d, i) {
                return _this.y(i) + (_this.itemHeight / 2) - (_this.strokeSize / 2);
            }).attr("r", 4).attr("data-value", function (d) {
                return d;
            }).attr("fill", function (d) {
                return baseColor.darker(colorDelta * d);
            });
        };
    ...
    D3Display.prototype._getItemData = function (logs) {
        ...
        _.each(data, function (d) {
            return d.positions.push(d.positions[d.positions.length - 1]);
        });
        return data;
    };
    return D3Display;
